...
ByteStream::ByteStream(const size_t capacity) : _queue(), _capacity_size(capacity) {}

size_t ByteStream::write(const string &data) {
    if (_end_input) {
        return 0;
    }
    size_t written_size = std::min(data.length(), remaining_capacity());
...
string ByteStream::peek_output(const size_t len) const {
    size_t peek_size = std::min(len, _queue.size());
